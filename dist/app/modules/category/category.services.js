...
const createCategory = async (payload) => {
    ...
    if (payload.parent) {
        const parentCategory = await category_model_1.Category.findById(payload.parent);
        if (!parentCategory) {
    ...
    const generatedSlug = payload.slug || payload.name
        ...
        .replace(/[\s_-]+/g, "-")
        .replace(/^-+|-+$/g, "");
    const existingCategory = await category_model_1.Category.findOne({ slug: generatedSlug });
    if (existingCategory) {
...
const getCategories = async (query) => {
    const categories = await category_model_1.Category.find().sort({ order: 1, name: 1 }).populate("parent");
    if (query.nested === "true") {
        return buildCategoryTree(categories);
    }
    return categories;
};
const getSingleCategory = async (identifier) => {
    if (mongoose_1.default.isValidObjectId(identifier)) {
        return await category_model_1.Category.findById(identifier).populate("parent");
    }
    return await category_model_1.Category.findOne({ slug: identifier }).populate("parent");
};
const updateCategory = async (id, payload) => {
    ...
    if (payload.name && !payload.slug) {
        const generatedSlug = payload.name
            ...
            .replace(/[\s_-]+/g, "-")
            .replace(/^-+|-+$/g, "");
        const existingCategory = await category_model_1.Category.findOne({ slug: generatedSlug, _id: { $ne: id } });
        if (existingCategory) {
            payload.slug = `${generatedSlug}-${Date.now()}`;
        }
        else {
...
const deleteCategory = async (id) => {
    ...
    await category_model_1.Category.updateMany({ parent: id }, { parent: null });
...
const buildCategoryTree = (categories, parentId = null) => {
    ...
    if (parentId === null) {
        filteredCategories = categories.filter((cat) => cat.parent === null || cat.parent === undefined);
    }
    else {
        filteredCategories = categories.filter((cat) => cat.parent && cat.parent._id.toString() === parentId.toString());
    }
